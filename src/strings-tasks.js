...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    ...
    for (let i = 0; i < str.length; i += 1) {
      const charUtf = str[i].charCodeAt();
      sum += charUtf;
    }
    return sum;
  }
...
function formatTime(minutes, seconds) {
  const minPadStard = minutes.toString().padStart(2, '0');
  const secPadStard = seconds.toString().padStart(2, '0');
  return `${minPadStard}:${secPadStard}`;
}
...
function isPalindrome(str) {
  const strNoSpaces = str.replaceAll(/[ ,!?]/g, '').toLowerCase();
  const strLeftSide = strNoSpaces.slice(
    0,
    Math.floor(strNoSpaces.length / 2 + 1)
  );
  const strRigthSide = strNoSpaces.slice(Math.ceil(strNoSpaces.length / 2 - 1));
  const strRigthSideReverse = strRigthSide.split('').reverse().join('');
  return strLeftSide === strRigthSideReverse;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    if (strArr[i].length > strRes.length) {
      strRes = strArr[i];
    }
  }
  return strRes;
...
function encodeToRot13(str) {
  ...
  const shiftedAlph = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (alph.includes(str[i])) {
      const indx = alph.indexOf(str[i]);
      res += shiftedAlph[indx];
    } else {
      res += str[i];
    }
  }
